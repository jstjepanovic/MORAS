    ...
    def next(self):
        ...
        while True:
            if len(self._line) == 0:
                ...
                i = self._line.find("//")
                v = self._line.find("*/")
                if (i != -1 and v == -1) or (v < i): self._line = self._line[:i]
                else: self._line = self._line[v:] 
    ...
    def _parseInt(self):
        ...
        while len(self._line):
            ...
            if c in numberChars:
                ...
                raise JackError('Syntax error in line ' + str(self._lineNum) + ': invalid int constant.')
            if ret > 32768:
                raise JackError('Syntax error in line ' + str(self._lineNum) + ': too large int constant.')
            self._line = self._line[1:]
        if n == True:
            ret *= -1
        return ret
